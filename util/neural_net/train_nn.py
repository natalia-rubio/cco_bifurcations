...
def train_nn(model, training_params):
    ...
    model_name = f"{model.output_type}_{model.anatomy}_ng_{model.num_geos}_nl_{model.num_layers}_lw_{model.layer_width}_ne_{training_params['num_epochs']}_bs_{training_params['batch_size']}_dr_{model.decay_rate}"
    plotting = True
    train_hist = []
    val_hist = []
