...
def solve_casadi_real(input_file = None, result_df = None):
...
if __name__ == "__main__":
    ...
    sol_prev = solve_casadi_real(input_file= input_file, result_df = df)
    
    df.sort_values(by=['name', 'time'], inplace=True)
    tree_name_split = tree_name.split("_")
    tree_name_split[-1] = "real"
    tree_name_real = "_".join(tree_name_split)   
    if not os.path.exists(f'trees/zerod_output/{junction_mode}/{tree_name_base}/{tree_name_real}'):
        os.makedirs(f'trees/zerod_output/{junction_mode}/{tree_name_base}/{tree_name_real}')
    df.to_csv(f'trees/zerod_output/{junction_mode}/{tree_name_base}/{tree_name_real}/sol_casadi.csv', index=False)
