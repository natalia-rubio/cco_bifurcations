...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'trees/zerod_input_{zerod_gen}_lookup_branch/{tree_name}_{flow_amp}'):
        os.makedirs(f'trees/zerod_input_{zerod_gen}_lookup_branch/{tree_name}_{flow_amp}')
    with open(f'trees/zerod_input_{zerod_gen}_lookup_branch/{tree_name}_{flow_amp}/solver_0d.json', 'w') as fp:
        json.dump(input_file, indent = 4, fp = fp)
    print(f"RRI 0D input file saved to trees/zerod_input_lookup_branch/{tree_name}_{flow_amp}/solver_0d.json")