...
def set_up_casadi_unsteady(time_step = 0, sol_prev = None, input_file = None, result_df = None):
    ...
    inlet_Q = input_file["boundary_conditions"][0]["bc_values"]["Q"][time_step]
    ...
    return (sol_dict, opti)

def update_casadi_unsteady(opti = casadi.Opti(), time_step = 0, sol_prev = None, input_file = None, result_df = None):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'trees/zerod_output/{junction_mode}/{tree_name_unsteady}'):
        os.makedirs(f'trees/zerod_output/{junction_mode}/{tree_name_unsteady}')
    df.to_csv(f'trees/zerod_output/{junction_mode}/{tree_name_unsteady}/sol_casadi.csv', index=False)
