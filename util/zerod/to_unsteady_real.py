...
if __name__ == "__main__":
    ...
    with open(f"trees/geo_files/{tree_name_base}/{tree_name_base}_flow_real/inflow_svFSI_flow_real.flow", 'r') as file:
        for line in file:
    ...
    #pdb.set_trace()
    t = t[1:]  # Remove the first time point
    Q = [q * inlet_area for q in Q[1:]]  # Scale flow by inlet area
    t = t[::10]  # Downsample time points
    Q = Q[::10]
    #pdb.set_trace()
    ...
    print(f"Standard 0D input file saved to trees/zerod_input/{junction_type}/{tree_name_base}/{tree_name}/solver_0d.json")