...
if __name__ == "__main__":
    ...
    resistance_dict = load_dict(f"data/tree_80_resistance_dict")
    num_junctions = 0
    num_non_bifs = 0
    out_of_dist_cnt = 0
    ...
    for junction in input_file["junctions"]:
        ...
        junction["junction_values"] = {"R_poiseuille": resistance_dict[junction_id], 
    ...
    if not os.path.exists(f'trees/zerod_input_{zerod_gen}_lookup/{tree_name}_{flow_amp}'):
        os.makedirs(f'trees/zerod_input_{zerod_gen}_lookup/{tree_name}_{flow_amp}')
    with open(f'trees/zerod_input_{zerod_gen}_lookup/{tree_name}_{flow_amp}/solver_0d.json', 'w') as fp:
        json.dump(input_file, indent = 4, fp = fp)
    print(f"RRI 0D input file saved to trees/zerod_input_lookup/{tree_name}_{flow_amp}/solver_0d.json")