...
def get_RRI_values(areas, tangents):
    ...
    if areas[1] < 0.01:
        pdb.set_trace()
    #print(f"angle diff: {(get_angle_diff(np.asarray(tangents[0]), np.asarray(tangents[1])), get_angle_diff(np.asarray(tangents[0]), np.asarray(tangents[2])))}")
...
def look_up_RRI_values(areas, tangents, tree_dict):
    ...
    # assert areas[2] == tree_dict[anatomy]["daughter2_area"][junction_number]
        ...
        r_lin_1 = tree_dict[anatomy]["r_lin_1"][i]
        r_lin_2 = tree_dict[anatomy]["r_lin_2"][i]
        r_quad_1 = tree_dict[anatomy]["r_quad_1"][i]
        r_quad_2 = tree_dict[anatomy]["r_quad_2"][i]
