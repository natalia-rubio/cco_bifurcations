...
def transform_standard_to_RI_full_fit(tree_name):
    ...
    if not os.path.exists(f'trees/zerod_output/RI_junctions_fit/{tree_name_base}/{tree_name}'):
        os.makedirs(f'trees/zerod_output/RI_full_fit/{tree_name_base}/{tree_name}')
    with open(f'trees/zerod_input/RI_full_fit/{tree_name_base}/{tree_name}/solver_0d.json', 'w') as fp:
