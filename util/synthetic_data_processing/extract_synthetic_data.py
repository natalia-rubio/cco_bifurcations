...
def extract_flow_behavior(geo_results_dir, offset):
    ...
    flow_ratio1 = [daughter1_flows[i]/inlet_flows[i] for i in range(len(daughter1_flows))]
    flow_ratio2 = [daughter2_flows[i]/inlet_flows[i] for i in range(len(daughter2_flows))]
    ...
    inlet_flow_stars = [inlet_flow/(U_char * A_char) for inlet_flow in inlet_flows]
    #pdb.set_trace()
    
    Q_star1 = np.asarray(daughter1_flow_stars).reshape(-1,)
    Q_star2 = np.asarray(daughter2_flow_stars).reshape(-1,)
    Q_star_inlet = Q_star1 + Q_star2#np.asarray(inlet_flow_stars).reshape(-1,)
    ...
    offset_dict["inlet_Re"] = [1.06*(flow/soln_dict["areas"][0,0])*np.sqrt(soln_dict["areas"][0,0]/np.pi)/0.04 for flow in inlet_flows]
    offset_dict["max_inlet_re"] = max(offset_dict["inlet_Re"])
    ...
    offset_dict["daughter1_flow_ratio"] = flow_ratio1[0]
    offset_dict["daughter2_flow_ratio"] = flow_ratio2[0]
    ...
    if verbose:
        ...
        print(f"Total inlet flow ratio: {flow_ratio1[0] + flow_ratio2[0]}")
