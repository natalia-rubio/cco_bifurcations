...
reader_0d_lookup = read_geo(f"trees/zerod_output_cent_{gen_mode}_lookup/{tree_name}_{flow_amp}/centerline_sol.vtp").GetOutput()
...
arrays_0d_lookup = get_all_arrays(reader_0d_lookup)
arrays_3d = get_all_arrays(reader_3d)

flow_lookup = arrays_0d_lookup["flow"]
flow_standard = arrays_0d_standard["flow"]
flow_3d = arrays_3d["Velocity"]

area = arrays_3d["CenterlineSectionArea"]

pressure_lookup = arrays_0d_lookup["pressure"]
pressure_standard = arrays_0d_standard["pressure"]
pressure_3d = arrays_3d["Pressure"]

energy_lookup = pressure_lookup + 0.5*1.06*flow_lookup**2/area
energy_standard = pressure_standard + 0.5*1.06*flow_standard**2/area
energy_3d = pressure_3d + 0.5*1.06*flow_3d**2/area


flow_error_lookup = (flow_lookup-flow_3d )/flow_3d
flow_error_lookup[arrays_3d["BifurcationId"] >= 0] = 0
print(f"Flow error lookup: {compute_rmse(flow_error_lookup,0*flow_error_lookup)}")
...
pressure_error_lookup = (pressure_lookup - pressure_3d)/pressure_3d
pressure_error_lookup[arrays_3d["BifurcationId"] >= 0] = 0
print(f"Pressure error lookup: {compute_rmse(pressure_error_lookup,0*pressure_error_lookup)}")
...
energy_error_lookup = (energy_lookup - energy_3d)/energy_3d
energy_error_lookup[arrays_3d["BifurcationId"] >= 0] = 0
print(f"Energy error lookup: {compute_rmse(energy_error_lookup,0*energy_error_lookup)}")
...
array = n2v(pressure_error_lookup)
array.SetName("lookup_pressure_error")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array) 

array = n2v(flow_error_lookup)
array.SetName("lookup_flow_error")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array) 

array = n2v(energy_error_lookup)
array.SetName("lookup_energy_error")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array)

array = n2v(flow_lookup)
array.SetName("lookup_flow")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array) 

array = n2v(pressure_lookup)
array.SetName("lookup_pressure")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array) 

energy_lookup[arrays_3d["BifurcationId"] >= 0] = 0
array = n2v(energy_lookup)
array.SetName("lookup_energy")
array.SetNumberOfValues(reader_3d.GetNumberOfPoints())
reader_3d.GetPointData().AddArray(array)
...
write_geo(f"trees/threed_output_cent/{tree_name}/centerline_sol_aug_lookup.vtp", reader_3d)
