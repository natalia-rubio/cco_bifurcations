...
def compare_to_3d_inlet_unsteady(junction_mode = "standard",
    ...
    times_0d = [float(key[9:]) for key in arrays_0d_0d.keys() if "pressure" in key]
    dt = times_0d[1] - times_0d[0]
    

    flows_0d = []; pressures_0d = []
    for time in times_0d:
        flows_0d.append(arrays_0d_0d[f"flow_{time:0.5f}"][arrays_3d["GlobalNodeId"] == 23][0])
        pressures_0d.append(arrays_0d_0d[f"pressure_{time:0.5f}"][arrays_3d["GlobalNodeId"] == 23][0]/1333)
    ...
    plt.clf()
    plt.plot(flows_0d, pressures_0d, label="0D standard")
    ...
    plt.clf()
    plt.plot(times_0d, pressures_0d)
