...
samples_uniform = uniform(loc=-0.5, scale=1).ppf(samples)
samples_normal = uniform(loc=0, scale=1).ppf(samples)
...
while success_counter < num_geos:
    ...
    while not area_consistency:
        for param_ind, param in enumerate(CCO_sampled_params_dict.keys()):
            if samples_uniform[i, param_ind] < 0:
                CCO_sampled_params_dict[param].append(stats_dict[param]["min"] * (1+samples_uniform[i, param_ind]))
            else:
                CCO_sampled_params_dict[param].append(stats_dict[param]["max"] * (0.8+samples_uniform[i, param_ind]))
