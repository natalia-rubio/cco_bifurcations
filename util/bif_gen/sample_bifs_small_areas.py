...
while success_counter < num_geos:
    ...
    while not area_consistency:
        for param_ind, param in enumerate(CCO_sampled_params_dict.keys()):
            if param == "daughter2_area_ratio":
                CCO_sampled_params_dict[param].append(
                0.1 + samples_uniform[i, param_ind] * 0.1)
            else:
                CCO_sampled_params_dict[param].append(
                stats_dict[param]["min"]*0.8 + samples_uniform[i, param_ind] * 1.4*(stats_dict[param]["max"] - stats_dict[param]["min"]))

        d1ar = CCO_sampled_params_dict["daughter1_area_ratio"][-1]
        d2ar = CCO_sampled_params_dict["daughter2_area_ratio"][-1]
        total_daughter_area_ratio = d1ar + d2ar
        print(d1ar, d2ar)
        if d2ar > 0.1 and d2ar < 0.2 and total_daughter_area_ratio > 0.8:
...
save_dict(CCO_sampled_params_dict, f"data/sampled_params_dict/{anatomy}_sampled_params_dict_small_areas2")
