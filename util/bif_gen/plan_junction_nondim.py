...
def plan_junction(geo_params):
    pts_per_branch = 4
    #tree_1
    branch_dict = {"branch1": {"branch_radii": [(1/3.14)**0.5, 
                                                (geo_params["daughter1_area_ratio"]/3.14)**0.5,], 
    ...
    for branch_name in ["branch1", "branch2"]:
        ...
        for i in range(num_segs):
            ...
            if branch_name == "branch1" and i == 0:
                ...
                length = geo_params["len1"]*L_char_nd/L_char #branch_radii[i] * 10
                pts_per_branch = int(length / (2*branch_radii[i]))
            else:
                length = geo_params["len2"]*L_char_nd/L_char
            ...
            if i != 0 or branch_name != "branch1":
                locs.append([end[0], end[1], end[2]])
                norms.append([end[0] - starts[i][0], end[1] - starts[i][1], end[2] - starts[i][2]])
                radii.append(float(branch_radii[i]))
