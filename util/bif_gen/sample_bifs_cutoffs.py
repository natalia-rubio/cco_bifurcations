...
while success_counter < num_geos:
    ...
    while not area_consistency:
        for param_ind, param in enumerate(CCO_sampled_params_dict.keys()):
                ...
                stats_dict[param]["min"]*0.6 + samples_uniform[i, param_ind] * 1.8*(stats_dict[param]["max"] - stats_dict[param]["min"]))

        d1ar = CCO_sampled_params_dict["daughter1_area_ratio"][-1]
        d2ar = CCO_sampled_params_dict["daughter2_area_ratio"][-1]
        print(d1ar, d2ar)
        if d2ar > 0.1 and d1ar>d2ar:
...
save_dict(CCO_sampled_params_dict, f"data/sampled_params_dict/{anatomy}_sampled_params_dict_wider")
