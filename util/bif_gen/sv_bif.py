...
tree_name = "tree_20"
set_type = "random_random2"
sampled_params_dict = load_dict("data/sampled_params_dict/{tree_name}_sampled_params_dict_random2".format(tree_name=tree_name))
...
for geo_index in range(len(sampled_params_dict["daughter1_angle"])):
    ...
    geo_dir = '/Users/natalia/Desktop/cco_bifurcations/data/synthetic_junctions/{}/{}/CCO_{:04d}'.format(tree_name, set_type, geo_index+1000)
    if os.path.exists(geo_dir):
        print("Geometry directory {:04d} already exists".format(geo_index+1000))
        continue
    else:
        os.mkdir(geo_dir)
    print("Generating geometry for CCO {:04d}".format(geo_index))
    #pdb.set_trace()
    geo_params={"daughter1_angle": sampled_params_dict["daughter1_angle"][geo_index],
                ...
                "L_char_3D": np.sqrt(1/np.pi),  # Assuming inlet_area_3D = 1
    ...
    print("Geometry {:02d} saved".format(geo_index+1000))
    #pdb.set_trace()
