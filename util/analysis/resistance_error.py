...
from util.tree.extract_true_junctions_helpers import add_0D_resistance, add_0D_RI_resistance, add_0D_solution_values, get_input_file_junction_dict_master, add_geometry_values, add_downstream_resistance_values, add_downstream_resistance_values_RI, add_solution_values, add_3D_resistance_total
...
def plot_resistances(tree_name):
    ...
    for junction_name, junction_dict in junction_dict_master.items():
        depth.append(junction_dict["depth"]+1)
    ...
    for depth_val in np.unique(depth):
        ...
        mean_0d_RI_err = np.mean(np.abs(res_0d_RI[idx] - res_3d[idx])/np.abs(res_3d[idx]))*100
        std_0d_RI_err = np.std(np.abs(res_0d_RI[idx] - res_3d[idx])/np.abs(res_3d[idx]))*100
        #pdb.set_trace()
    ...
    axs[1].set_xlabel("Depth ($\#$ bifurcations downstream)")
