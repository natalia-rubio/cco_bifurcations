...
for junction_mode in junction_mode_list:
    total_err = 0; rel_err = 0; count = 0
    for tree_name in tree_list:
        for flow_mag in flow_mag_list:
            if error_dict[tree_name][flow_mag][junction_mode] is None:
                continue
            total_err += np.mean(np.abs(error_dict[tree_name][flow_mag][junction_mode]["pressure_error_0d_tot"]))
            rel_err += np.mean(np.abs(error_dict[tree_name][flow_mag][junction_mode]["pressure_error_0d_rel"]))
            count += 1
    print(f"Average total error for {junction_mode}: {total_err/count:.2f} mmHg")
    print(f"Average relative error for {junction_mode}: {rel_err/count:.2f} %")
# plt.clf()
...
for i, middle in enumerate(flow_mag_list):
    ...
    for j, inner in enumerate(junction_mode_list):
        ...
        points = [np.abs(error_dict[outer][middle][inner]["pressure_error_0d_tot"][0]) for outer in tree_list]
        ...
        ax2.scatter(bar_positions, points, color="black", marker='o', label=f"Total Error" if i == 0 and j== 0 else "", s=20,)
...
ax2.legend(loc='upper center', bbox_to_anchor=(0.5, 2.0), ncol =1 , frameon=False)

# Labels and title
ax.set_ylabel('Inlet Pressure Error (\%)')
ax2.set_ylabel('Inlet Pressure Error (mmHg)')
ax2.set_ylim(0, 240)
# Show plot
plt.tight_layout()
