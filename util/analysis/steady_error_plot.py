...
hatch_list = ["xxx", "...", ""]
color_list_light = ["lightblue", "lightgreen", "lightyellow"]
...
for i, middle in enumerate(flow_mag_list):
    ...
    for j, inner in enumerate(junction_mode_list):
        ...
        heights = [np.abs(error_dict[outer][middle][inner]["pressure_error_0d_rel"][0]) for outer in tree_list]
        bar_positions = indices*(((num_junction_modes+1)*len(flow_mag_list)+1)*bar_width) + offset - (bar_width * num_junction_modes * len(flow_mag_list)+1) / 2
        if i == j == 0:
...
for t, tree_name in enumerate(tree_list):     
    for i, middle in enumerate(flow_mag_list):
        tick_list.append((i * (num_junction_modes+1)) * bar_width + (t * (3 * (num_junction_modes+1) + 1) * bar_width) + start_pos + bar_width)
        re_list.append(f"Re = {round(int(error_dict[tree_list[t]][middle]['RR']['inlet_re'][0]), -2)}")
...
ax.set_ylabel('Inlet Pressure Percentage Error (\%)')
